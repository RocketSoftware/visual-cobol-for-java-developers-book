...
public class CustomerController {
	...
	public ArrayWrapper<CustomerForm> customers() {
		...
		try (RunUnit<CustomerDataAccess> ru = new RunUnit<>(CustomerDataAccess.class)) {
			try (CustomerDataAccess accessor = (CustomerDataAccess) ru.GetInstance(CustomerDataAccess.class, true)) {
	...
	@GetMapping(value = "/{id}")
	public CustomerForm getCustomer(@PathVariable("id") int id) throws ResourceNotFoundException {
		try (RunUnit<CustomerDataAccess> ru = new RunUnit<>(CustomerDataAccess.class)) {
			try (CustomerDataAccess accessor = (CustomerDataAccess) ru.GetInstance(CustomerDataAccess.class, true)) {
				accessor.open(AbstractBusinessAccess.OpenMode.read);
				...
				if (dto != null) {
					...
					throw new ResourceNotFoundException(String.format("Could not find record %d", id));
				}
			}
		}
	}
	...
	@PostMapping
	public ResponseEntity<CustomerForm> addCustomer(@RequestBody CustomerForm customer) {
		try (RunUnit<CustomerDataAccess> ru = new RunUnit<>(CustomerDataAccess.class)) {
			try (CustomerDataAccess accessor = (CustomerDataAccess) ru.GetInstance(CustomerDataAccess.class, true)) {
	...
	public ResponseEntity<?> deleteCustomer(@PathVariable("id") int id) throws ResourceNotFoundException {
		HttpStatus status = null;
		try (RunUnit<CustomerDataAccess> ru = new RunUnit<>(CustomerDataAccess.class)) {
			try (CustomerDataAccess accessor = (CustomerDataAccess) ru.GetInstance(CustomerDataAccess.class, true)) {
				accessor.open(AbstractBusinessAccess.OpenMode.write); 
				status = accessor.deleteCustomer(id) ? HttpStatus.NO_CONTENT : HttpStatus.NOT_FOUND;
			}
		}
	...
	@PutMapping
	public ResponseEntity<String> updateCustomer(@RequestBody CustomerForm customer) {
		try (RunUnit<CustomerDataAccess> ru = new RunUnit<>(CustomerDataAccess.class)) {
			try (CustomerDataAccess accessor = (CustomerDataAccess) ru.GetInstance(CustomerDataAccess.class, true)) {
				accessor.open(AbstractBusinessAccess.OpenMode.rw);
				CustomerDto dto = customer.createCustomerDto();
				HttpStatus status = accessor.updateCustomer(dto) ? HttpStatus.NO_CONTENT : HttpStatus.NOT_FOUND;
