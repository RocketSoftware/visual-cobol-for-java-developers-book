      ...
      *> and an account record is neeed as the foreign key to a transaction
       add-fk-records section.
           ...
           if failed perform test-failed end-if
           move FK-ACCOUNT-ID to WS-ACCOUNT-ID of WS-ACCOUNT
           move FK-CUSTOMER-ID to WS-CUSTOMER-ID of WS-ACCOUNT
           move FK-BALANCE to WS-BALANCE
           move FK-CREDIT-LIMIT to WS-CREDIT-LIMIT
           move FK-TYPE  to WS-TYPE
           call ADD-ACCOUNT using by reference function-status
                                               WS-ACCOUNT
           if failed perform test-failed end-if
           move FK-ACCOUNT-ID-2 to WS-ACCOUNT-ID of WS-ACCOUNT
           move FK-CUSTOMER-ID to WS-CUSTOMER-ID of WS-ACCOUNT
           move FK-BALANCE to WS-BALANCE
           move FK-CREDIT-LIMIT to WS-CREDIT-LIMIT
           move FK-TYPE  to WS-TYPE
